...
def validate_scenario(scenario: Path, main_eval_root: Path | None) -> list[str]:
    ...
    if is_main_eval and task_type == "implementation":
        ...
        if category_scores["optional_quality"] <= 0:
            failures.append(f"{criteria_file}: main eval implementation scenario needs optional_quality criteria")
    elif is_main_eval and task_type == "cleanup" and category_scores["optional_quality"] <= 0:
        failures.append(f"{criteria_file}: main eval cleanup scenario needs optional_quality criteria")
...
def main() -> int:
    ...
    if main_eval_root and any(path.resolve() == main_eval_root.resolve() for path in paths if path.exists()):
        ...
        if main_eval_total:
            optional_quality = main_eval_category_scores["optional_quality"]
            safety = main_eval_category_scores["safety"]
            maintainability = main_eval_category_scores["maintainability"]
            if optional_quality < main_eval_total * 0.8:
                failures.append(
                    "evals: main eval set should be primarily Optional-quality scoring "
                    f"({optional_quality}/{main_eval_total})"
                )
