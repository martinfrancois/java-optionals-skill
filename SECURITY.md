...
- the affected commit, release, or Tessl tile version;
...
- if the report affects a released tile, evidence from the published tile version;
...
Security reports should show impact caused by this repository, the published Tessl tile, or this
...
If the same issue would affect any Tessl tile, any GitHub Actions repository, any package-manager
...
- GitHub Actions, release, Renovate, or Tessl publish behavior that lets untrusted code run with
  secrets or publish a tile without the expected checks;
...
- dependency or toolchain vulnerabilities with a working reproduction that shows impact through this
  repository, its CI, its release process, or the published Tessl tile;
- an upstream Tessl, GitHub, package-manager, Java, or coding-agent issue that becomes exploitable
  here because of this repository's configuration or published tile content.
...
- Tessl CLI, Tessl registry, or Tessl account issues that affect all tiles in the same way and don't
  depend on this tile's content, metadata, CI, or release setup;
